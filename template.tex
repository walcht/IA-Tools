...
\begin{itemize}
	...
	\item \textbf{Free Roaming Shared Environment} - To emulate a physically
	      ...
	      (\ref{fig:fiesta} (3)). The same unique color of the user's avatar
...
visualizations around them in an egocentric layout (\ref{fig:fiesta} (4)) and
...
widgets placed on top of it (\ref{fig:uplift} (2)). Uplift also makes use of
tangible widgets which are physical and interactive elements that control
visualization parameters (for instance by affecting sliders) (\ref{fig:uplift}
...
(\ref{fig:uplift} (1)). If these models are picked up, AR will overlay
relevant data about them (\ref{fig:uplift} (6)). AR is used to display
multiple 3D data types on top of the tabletop and 2D graphs alongside legends
around it (\ref{fig:uplift} (5)). On top of these, Uplift uses a large display
to either replicate the content of the tabletop or show additional
visualizations (\ref{fig:uplift} (2)).
...
a clear signaling of transition status either through visual (\ref{fig:hybridaxes} (2, 5)) or
...
\begin{itemize}
	\item \textbf{Pull visualizations from the 2D desktop} - by using free-hand gestures
	      or the controller(s) (\ref{fig:hybridaxes} (1, 2, 3)).
	\item \textbf{Push visualizations from the immersive environment back to the 2D
		      desktop} (\ref{fig:hybridaxes} (4, 5, 6)).
